...
export default function UserPage() {
  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        setError("Failed to load user. The server might be starting up. Please try again later.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const fetchAvailableBadges = async () => {
    try {
      ...
      if (res?.success) {
        ...
        const userBadgeIds = user?.badges?.map(userBadge => userBadge.badgeId._id) || [];
        
        // Filter out badges that user already has
        const availableBadges = res.data.filter(badge => !userBadgeIds.includes(badge._id));
  ...
  const handleBadgeSubmit = async (e) => {
    ...
    try {
      ...
      const res = await postData(`/badges/${badgeFormData.badgeId}/assign`, payload);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res?.success) {
        ...
        setModalOpen(false);
        toast.success(isEditMode ? 'Rating updated successfully!' : 'Rating added successfully!');
      } else {
        toast.error(isEditMode ? 'Failed to update rating. Please try again.' : 'Failed to add rating. Please try again.');
      }
    } catch (err) {
      console.error(isEditMode ? "Error updating rating:" : "Error adding rating:", err);
      toast.error(isEditMode ? 'Error updating rating. Please try again.' : 'Error adding rating. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <>
        ...
        <div className="max-w-7xl mx-auto px-6 py-8">
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            <div className="border-l-4 border-blue-600 pl-4">
              ...
              <p className="text-gray-600">Comprehensive user profile and performance tracking</p>
            </div>
          </div>
                ...
                <div className="p-6">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-6">
                      <div className="relative">
                        <img
                          src={user.imageUrl?.trim() || "https://via.placeholder.com/100"}
                          alt={user.name}
                  ...
                  <div className="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      { label: 'Group', value: user.groupName, color: 'purple' },
                      { label: 'Joined', value: new Date(user.createdAt).toLocaleDateString(), color: 'green' },
                      { label: 'Last Updated', value: new Date(user.updatedAt).toLocaleDateString(), color: 'orange' }
                    ].map((item, index) => (
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 bg-gray-50 border-b border-gray-200 flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-gray-600 text-sm mt-1">Recognition and accomplishments</p>
                  </div>
                  <button
                ...
                <div className="p-6">
                  {user.badges && user.badges.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <h4 className="text-lg font-medium text-gray-900 mb-2">No badges assigned</h4>
                      <p className="text-gray-500 mb-4">Award badges to recognize achievements and milestones</p>
                      <button
                        onClick={handleAddBadge}
                        className="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm bg-blue-600 text-sm font-medium text-white hover:bg-blue-700 transition-colors duration-200"
                      >
              ...
              {user.assignedMonths && user.assignedMonths.length > 0 && (
                <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                  <div className="px-6 py-4 bg-gray-50 border-b border-gray-200">
                    ...
                    <p className="text-gray-600 text-sm mt-1">Monthly assignments and daily ratings</p>
                  </div>
                  <div className="p-6">
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">
                {isEditMode ? 'Edit Rating' : 'Add Rating'} for Day {selectedDay?.dayNumber}
              </h3>
              <p className="text-sm text-gray-600 mt-1">
                Date: {selectedDay && new Date(selectedDay.date).toLocaleDateString()}
              </p>
            </div>
                    ...
                    {[
                      ...
                      <label key={option.value} className="flex items-center cursor-pointer">
                        <input
                          ...
                          onChange={() => handleInputChange('assignmentStatus', option.value)}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleUserSubmit} className="space-y-6">
                    ...
                    <div className="mb-3 flex justify-center">
                      <div className="relative">
                        ...
                        <button
                          ...
                          onClick={() => {
                            clearImageSelection();
                            setUserFormData(prev => ({ ...prev, imageUrl: "" }));
                          }}
                ...
                {[
                  ...
                  <div key={field}>
                    ...
                    <input
                      ...
                      onChange={(e) => handleUserInputChange(field, e.target.value)}
                      className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                      required
                    />
                  </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => handleUserInputChange('imageUrl', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
            ...
            <div className="px-6 py-4 bg-gray-50 border-t border-gray-200 flex justify-end space-x-3">
              ...
              >
                ...
                {submitting ? 'Updating...' : isUploading ? 'Uploading...' : 'Update User'}
              </button>
            </div>
          </div>
        </div>
      ...
      {badgeModalOpen && (
        <div className="fixed inset-0 bg-gray-900/70 bg-opacity-85 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="px-6 py-4 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Assign Badge to User</h3>
            </div>
            <div className="px-6 py-4">
              <form onSubmit={handleBadgeSubmit} className="space-y-6">
                ...
                <div>
                  ...
                  {badgeFormData.badgeId && (
                    <div className="mt-3 p-4 bg-gray-50 rounded-md border border-gray-200">
                      {(() => {
                        const selectedBadge = availableBadges.find(b => b._id === badgeFormData.badgeId);
                        return selectedBadge ? (
                          <div className="space-y-2 text-sm">
                            <div className="flex justify-between">
                              <span className="font-medium text-gray-600">Category:</span>
                              <span className="capitalize text-gray-900">{selectedBadge.category}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="font-medium text-gray-600">Status:</span>
                              <span className={`font-medium ${selectedBadge.isActive ? 'text-green-600' : 'text-red-600'}`}>
                                {selectedBadge.isActive ? 'Active' : 'Inactive'}
                              </span>
                            </div>
                            <div className="pt-2 border-t border-gray-200">
                              <span className="font-medium text-gray-600">Requirements:</span>
                              <p className="text-gray-700 mt-1">{selectedBadge.requirements}</p>
                            </div>
                          </div>
                        ) : null;
                      })()}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) => handleBadgeInputChange('notes', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500"
                    rows="3"
        ...
        <div className="border-t border-gray-200 pt-3">
          <div className="grid grid-cols-2 gap-3 text-xs">
            ...
            <div>
              ...
              <p className="font-medium text-gray-900 mt-1">{new Date(userBadge.awardedDate).toLocaleDateString()}</p>
            </div>
          </div>
          {userBadge.notes && (
            <div className="mt-3 pt-3 border-t border-gray-200">
              <span className="text-gray-500 text-xs">Notes:</span>
              <p className="text-gray-700 text-sm mt-1 italic">"{userBadge.notes}"</p>
            </div>
          )}
        </div>
      </div>
...
function MonthCard({ month, onAddRating, onEditRating }) {
  const monthNames = [
    "January", "February", "March", "April", "May", "June",
    "July", "August", "September", "October", "November", "December"
  ];

  const monthName = monthNames[month.monthNumber - 1] || `Month ${month.monthNumber}`;
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <div className="flex items-center space-x-2">
              ...
              <h4 className="text-lg font-semibold text-gray-900">{monthName} {month.year}</h4>
            </div>
...
// Week Card Component
function WeekCard({ week, onAddRating, onEditRating }) {
  return (
    <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
      <div className="flex justify-between items-center mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h5 className="text-md font-medium text-gray-900">Week {week.weekNumber}</h5>
        </div>
...
// Day Card Component
function DayCard({ day, onAddRating, onEditRating }) {
    ...
    <div className="bg-white border border-gray-200 rounded-lg p-3 shadow-sm hover:shadow-md transition-shadow duration-200">
      <div className="mb-3">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className={`w-3 h-3 rounded-full ${hasRating ? 'bg-green-500' : 'bg-gray-300'}`}></div>
        </div>
